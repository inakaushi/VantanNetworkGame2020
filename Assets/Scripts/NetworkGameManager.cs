...
public class NetworkGameManager : MonoBehaviourPunCallbacks
{
    ...
    public static NetworkGameManager Instance
    {
        get
        {
            if (instance == null)
            {
                instance = (NetworkGameManager)FindObjectOfType(typeof(NetworkGameManager));

                if (instance == null)
                {
                    Debug.LogError("NetworkGameManager をアタッチしているGameObjectはありません");
                }
            }
    ...
    private void JoinLobby()
    {
        if (PhotonNetwork.IsConnected)
        {
            PhotonNetwork.JoinLobby();  //デフォルトロビーに接続
        }
    }
    ...
    private void SpawnPlayerInLobby()
    {
        ...
        MapManager.Instance.CreatePlayer(actorNumber - 1);
